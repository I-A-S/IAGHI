<div align="center">
  <img src="logo.png" alt="IAGHI Logo" width="240" style="border-radius: 1.15rem;"/>
  <br/>
  <br/>

  <img src="https://img.shields.io/badge/standard-C%2B%2B20-darkred.svg" alt="C++ Standard"/>
  <img src="https://img.shields.io/badge/compiler-MSVC_|_Clang-darkgreen.svg" alt="Compiler"/>

  <p style="padding-top: 0.2rem;">
    <b>IAGHI: IA Graphics Hardware Interface</b>
  </p>
</div>

## **Overview**
**IAGHI** is a modern, lightweight, and explicit C++ graphics abstraction layer built over Vulkan. It is designed to significantly reduce the boilerplate required to render modern 3D graphics while maintaining the performance characteristics and explicit control of low-level APIs.

Currently, IAGHI fully supports standard graphics rendering and compute.

## **Features**
- **Clean C++ API**: Utilizes modern C++ features, strong typing, and the Auxid `Result<T>` pattern for robust error handling.
- **Simplified Resource Management**: Batch creation and destruction of Buffers, Images, and Samplers to minimize driver overhead.
- **Streamlined Synchronization**: Replaces Vulkan's verbose memory barriers, access flags, and pipeline stages with a simplified `EResourceState` approach, while still allowing precise subresource control.
- **Dynamic Descriptor Tables**: Easy-to-use binding layouts and descriptor updates for uniforms, storage buffers, and sampled images, with configurable descriptor pool sizes.
- **Modern Rendering**: Supports SPIR-V shaders directly.
- **Off-screen Rendering**: Full support for headless rendering into off-screen buffers without requiring a window surface.
- **Render to Texture**: Render targets allow rendering to any texture for shadow mapping, post-processing, and more.
- **Robust Device Selection**: Automatically prioritizes discrete hardware but seamlessly falls back to integrated or virtual GPUs when necessary.
- **Texture & Mipmap Helpers**: Helper functions for automatic mipmap generation (with automatic nearest-neighbor filtering fallback) and handling block-compressed texture formats.

## **Architecture & Philosophy**

IAGHI provides an opaque handle-based architecture (e.g., `Device`, `Buffer`, `Pipeline`, `CommandBuffer`). It strips away the hundreds of lines of setup code usually required for Vulkan, allowing you to focus on the actual rendering logic.

There are no implicit render passes; the API is designed around modern dynamic rendering paradigms, utilizing `cmd_begin_rendering()` to render seamlessly to swapchains or off-screen textures.

## **Examples**
IAGHI includes several examples to help you get started:
- **Hello Triangle** (`examples/hello-triangle`): A basic example demonstrating device initialization, resource creation, and simple on-screen rendering.
- **Rotating Cube** (`examples/rotating-cube`): Demonstrates 3D rendering with uniform buffers, push constants, and depth testing.
- **Off-screen Rendering** (`examples/offscreen-rendering`): Shows how to perform headless rendering into an off-screen buffer without requiring a window surface.
- **Hello Compute** (`examples/hello-compute`): Shows how to use the compute pipeline to process data in a headless compute shader without drawing.
- **Render to Texture** (`examples/render-to-texture`): Demonstrates rendering to an off-screen texture and using it in a subsequent rendering pass.

## **Quick Look**

Here is a conceptual overview of how to interact with the API:

### 1. Device Initialization
   
```cpp
ghi::InitInfo init_info = {};
init_info.app_name = "My IAGHI App";
init_info.validation_enabled = true;
init_info.surface_width = 1920;
init_info.surface_height = 1080;
// ... populate window surface callbacks ...

auto device = ghi::create_device(init_info).value();
```

### 2. Resource Creation

```cpp
// Create a buffer
ghi::Buffer vertex_buffer;
ghi::BufferDesc vbo_desc = {
    .size_bytes = sizeof(vertices),
    .usage = ghi::EBufferUsage::Vertex,
    .cpu_visible = true,
    .debug_name = "Main VBO"
};
ghi::create_buffers(device, {vbo_desc}, {vertex_buffer});

// Upload data
void* mapped = ghi::map_buffer(device, vertex_buffer);
memcpy(mapped, vertices, sizeof(vertices));
ghi::unmap_buffer(device, vertex_buffer);
```

### 3. The Render Loop

```cpp
while (!window_should_close)
{
    // Acquires the swapchain image and begins recording
    ghi::CommandBuffer cmd = ghi::begin_frame(device);

    ghi::cmd_begin_pipeline(cmd, main_pass_pipeline);
    
    ghi::ColorAttachment color_attachment{
        .texture = nullptr, // nullptr means render to swapchain
        .clear_color = {0.1f, 0.1f, 0.1f, 1.0f},
        .load_op_clear = true,
        .store_op_store = true,
    };
    
    ghi::RenderingInfo rendering_info{
        .width = 1920,
        .height = 1080,
        .color_attachments = {color_attachment},
        .depth_attachment = nullptr,
    };
    
    ghi::cmd_begin_rendering(device, cmd, rendering_info);

    ghi::cmd_set_viewport(cmd, 0, 0, 1920.0f, 1080.0f);
    ghi::cmd_set_scissor(cmd, 0, 0, 1920, 1080);
    
    ghi::cmd_bind_descriptor_table(cmd, 0, main_pass_pipeline, global_descriptor_table, {});
    ghi::cmd_bind_frame_bound_descriptor_table(cmd, 1, main_pass_pipeline, per_frame_descriptor_table);

    ghi::cmd_push_constants(cmd, main_pass_pipeline, 0, sizeof(pc), &pc);
    
    ghi::cmd_bind_vertex_buffers(cmd, 0, {vertex_buffer}, {0});
    ghi::cmd_bind_index_buffer(cmd, index_buffer, 0, true);

    ghi::cmd_draw_indexed(cmd, index_count, 1, 0, 0, 0);
    
    ghi::cmd_end_rendering(cmd);
    ghi::cmd_end_pipeline(cmd, main_pass_pipeline);

    // Submits the command buffer and presents to the screen
    ghi::end_frame(device);
}

ghi::wait_idle(device);
```

## **Roadmap**
The graphics core is complete! The next major milestones for IAGHI are:

**Core**
- [ ] **WebGPU Backend**: Implement WebGPU backend for IAGHI.

**Tooling**
- [ ] **Pipeline Baker**: A tool to generate pipeline layouts and descriptor sets from JSON.

## **Contributing**
Issues and feature requests are very welcome! However, contributions to the core code are not currently being accepted.

## **License**

Copyright © 2026 IASoft (PVT) LTD. Licensed under the [PolyForm Noncommercial License 1.0.0](https://polyformproject.org/licenses/noncommercial/1.0.0).
